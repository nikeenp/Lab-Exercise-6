...
def print_inventory_items(items):
    ...
    """
    names_1 = list_of_items(items)
    print("You have " + names_1 + ".\n")
...
def execute_go(direction):
    ...
    if is_valid_exit(current_exits, direction):                                                                                                                                             
        current_room = move(current_exits, direction)
    else:
...
def execute_take(item_id):
    ...
    for item in current_room["items"]:
        if item["id"] == item_id:
            if mass_of_inventory() + float(item["mass"])<3.0:
                current_room["items"].remove(item)
                inventory.append(item)
                found = True
            else:
                found = True
                print("You cannot take that, you will have too much to carry.")
    if not found:
...
def main():
    ...
    while not Win:
        ...
        reception_items = rooms["Reception"]["items"]
        if len(reception_items) == 6:
            Win = True
    print("YOU WIN!")
